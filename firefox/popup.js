...
async function openPreviousHistory() {
  ...
  try {
    const result = await browser.tabs.sendMessage(tab.id, { type: "cg-open-window-history" });
    if (result && result.ok) {
      window.close();
      return;
    }
...
async function initialize() {
  ...
  const tab = await currentTab();
  if (tab && tab.id != null) {
    renderStats(await browser.runtime.sendMessage({ type: "cg-get-stats", tabId: tab.id }));
  }
}
