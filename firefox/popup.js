...
async function saveSettings() {
  try {
    return await browser.runtime.sendMessage({
      ...
      stallRecoveryTimeoutSeconds: stallRecoveryTimeoutSeconds()
    });
  } catch (error) {
...
async function initialize() {
  const saved = await browser.storage.local.get({ enabled: false, mode: "windowed-visible", maxDisplayMessages: 64, showGuardNotice: true, stallRecoveryEnabled: true, stallRecoveryTimeoutSeconds: 120, cgTotals: EMPTY_TOTALS, cgLastIssue: null });
