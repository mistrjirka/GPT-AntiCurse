...
# For a long conversation, also require the private-history/trim path:
CHATGPT_SMOKE_REQUIRE_TRIM=1 CHATGPT_SMOKE_URL='https://chatgpt.com/c/<conversation-id>' node tests/live-chatgpt-smoke.js
```

For an unauthenticated browser where anonymous chat creation is available, `CHATGPT_SMOKE_CREATE_ANON=1 node tests/live-chatgpt-smoke.js` creates one tiny throwaway chat and reloads it. A copied authenticated Chromium profile can be supplied with `CHATGPT_SMOKE_USER_DATA_DIR`. The smoke fails if the real detail endpoint is not observed, AntiCurse does not report matching interception stats, or the live response shape is rejected. With `CHATGPT_SMOKE_REQUIRE_TRIM=1`, it additionally requires `nativeTrimConfirmed`, private history, and the matching history conversation ID. A login redirect or Cloudflare/bot challenge is a blocked release gate, not a fixture pass.
