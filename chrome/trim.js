...
(function (global) {
  ...
  function leadingPrefix(mapping, chain, maxPrefixNodes) {
    ...
    for (let i = 0; i < chain.length && prefix.length < maxPrefixNodes; i++) {
      if (isDisplayCandidate(mapping[chain[i]])) break;
      prefix.push(chain[i]);
    }
    return prefix;
  }
  ...
  function selectRecent(mapping, chain, cfg) {
    ...
    for (let i = chain.length - 1; i >= 0; i--) {
      if (isDisplayCandidate(mapping[chain[i]])) {
        seenDisplay++;
        if (seenDisplay >= cfg.maxDisplayMessages) { cutoff = i; break; }
      }
    }
  ...
  function rebuildLinearMapping(data, keptChain) {
    ...
    for (let i = 0; i < keptChain.length; i++) {
      ...
      newMapping[id] = Object.assign({}, mapping[id], {
  ...
  global.CGTrim = { trimConversation, DEFAULTS, isDisplayCandidate, isExplicitlyHidden };
