...
(() => {
  ...
  function attachNativeWatch() {
    ...
    const next = findNativeScroller(); if (!next) return false;
    ...
    detachNativeWatch(); nativeScroller = next; nativeEventTarget = target; lastNativeTop = nativeTop(); autoArmed = lastNativeTop > 64;
  ...
  function ensureNativeWatch() {
    ...
    attachObserver = new MutationObserver(() => { if (attachNativeWatch()) { attachObserver.disconnect(); attachObserver = null; } });
  ...
  function ensureOverlay() {
    ...
    host = document.createElement("div"); host.id = "cg-window-history-host"; host.style.cssText = "all:initial;position:fixed;inset:0;z-index:2147483646;pointer-events:none;";
    document.documentElement.appendChild(host); shadow = host.attachShadow({ mode:"closed" });
    shadow.innerHTML = `<style>:host{all:initial}.overlay{position:fixed;inset:0;display:none;pointer-events:auto;background:#111113;color:#ececec;font:14px/1.55 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.overlay.open{display:flex;flex-direction:column}.topbar{display:flex;align-items:center;gap:8px;padding:9px 14px;border-bottom:1px solid rgba(255,255,255,.09);background:#171719}.brand{font-weight:700;color:#67e8d3}.marker{flex:1;min-width:0;color:#92969e;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}button{border:1px solid rgba(255,255,255,.13);border-radius:8px;background:#242427;color:#f3f4f6;padding:6px 9px;font:600 12px system-ui;cursor:pointer}button:hover{background:#303034}button:disabled{opacity:.4;cursor:default}.viewport{flex:1;min-height:0;overflow:auto;overscroll-behavior:contain}.list{width:min(52rem,calc(100% - 32px));margin:auto;padding:18px 0 54px;display:grid;gap:16px}.turn{padding:10px 14px;border-radius:13px;overflow-wrap:anywhere}.turn.user{margin-left:min(14%,5rem);background:#2b2b2e}.turn.assistant{margin-right:min(8%,3rem)}.head{color:#6ee7d2;font-size:11px;font-weight:700;margin-bottom:5px}.body{white-space:pre-wrap}.hint{width:max-content;max-width:calc(100% - 32px);margin:12px auto 0;color:#777f88;font-size:11px;text-align:center}</style><section class="overlay" role="dialog" aria-label="Older visible ChatGPT history"><div class="topbar"><span class="brand">AntiCurse history</span><span class="marker"></span><button class="previous" type="button"></button><button class="back" type="button">Back to recent</button></div><div class="viewport" tabindex="0"><div class="hint">Visible user/assistant turns only. Scroll to page through older history.</div><div class="list"></div></div></section>`;
    ...
    previousButton.addEventListener("click", prependOlder); shadow.querySelector(".back").addEventListener("click", closeOverlay); viewport.addEventListener("scroll", onOverlayScroll, { passive:true }); viewport.addEventListener("wheel", onOverlayWheel, { passive:false }); viewport.addEventListener("keydown", e => { if (e.key === "Escape") closeOverlay(); });
  }
  function updateMarker() {
    if (!marker || !history) return; const shown = Math.max(0, loadedEnd - loadedStart); marker.textContent = `${shown.toLocaleString()} shown · ${Math.max(0, loadedStart + 1).toLocaleString()}–${loadedEnd.toLocaleString()} of ${nativeStart.toLocaleString()} older turns`;
  ...
  function openOverlay() {
    if (isOpen || !history || !LIMITED_MODES.has(settings.mode)) return false; nativeStart = Math.max(0, history.messages.length - Math.max(0, Number(history.nativeVisibleCount) || 0)); if (nativeStart <= 0) return false;
    ensureOverlay(); renderInitialWindow(); isOpen = true; host._cgOverlay.classList.add("open"); requestAnimationFrame(() => { viewport.scrollTop = viewport.scrollHeight; try { viewport.focus({ preventScroll:true }); } catch (_) { viewport.focus(); } }); return true;
  }
  function closeOverlay() { if (!isOpen) return; isOpen = false; host?._cgOverlay?.classList.remove("open"); suppressAutoUntil = performance.now() + 700; autoArmed = false; if (nativeScroller) { nativeScroller.scrollTop = 0; lastNativeTop = 0; } }
  function prependOlder() {
    if (!history || loadedStart <= 0) return false; const next = Math.max(0, loadedStart - pageSize()), anchor = list.firstElementChild, before = anchor?.getBoundingClientRect().top || 0, f = document.createDocumentFragment();
    for (let i = next; i < loadedStart; i++) f.appendChild(makeTurn(history.messages[i], i)); list.insertBefore(f, list.firstChild); loadedStart = next; while (list.children.length > maxRendered()) { list.lastElementChild.remove(); loadedEnd--; } updateMarker(); if (anchor) viewport.scrollTop += anchor.getBoundingClientRect().top - before; return true;
  }
  function appendNewer() {
    if (!history || loadedEnd >= nativeStart) return false; const next = Math.min(nativeStart, loadedEnd + pageSize()), anchor = list.lastElementChild, before = anchor?.getBoundingClientRect().top || 0, f = document.createDocumentFragment();
    for (let i = loadedEnd; i < next; i++) f.appendChild(makeTurn(history.messages[i], i)); list.appendChild(f); loadedEnd = next; while (list.children.length > maxRendered()) { list.firstElementChild.remove(); loadedStart++; } updateMarker(); if (anchor) viewport.scrollTop += anchor.getBoundingClientRect().top - before; return true;
  }
  function onOverlayScroll() { if (!isOpen || scrollBusy) return; scrollBusy = true; requestAnimationFrame(() => { scrollBusy = false; if (!isOpen) return; if (viewport.scrollTop < 260) prependOlder(); const gap = viewport.scrollHeight - viewport.clientHeight - viewport.scrollTop; if (gap < 260 && loadedEnd < nativeStart) appendNewer(); }); }
  ...
  function onNativeScroll() { if (!history || isOpen || !settings.enabled || settings.mode !== "windowed-visible" || !nativeScroller) return; const current = nativeTop(); if (current > 64) autoArmed = true; const movingUp = current < lastNativeTop - .5; if (autoArmed && movingUp && current <= 10 && nativeScrollable() && performance.now() >= suppressAutoUntil) openOverlay(); lastNativeTop = current; }
  ...
  ext.runtime.onMessage.addListener((message, _sender, sendResponse) => { if (message?.type === "cg-open-window-history") { const result = manualOpen(); if (IS_FIREFOX) return Promise.resolve(result); sendResponse?.(result); return false; } if (IS_FIREFOX && message?.type === "cg-window-history") applyHistory(message.history); return undefined; });
