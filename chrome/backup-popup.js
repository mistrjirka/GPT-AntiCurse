...
(() => {
  ...
  function buttons(on) {
    ...
    continueButton.disabled = !on;
  }

  async function tab() {
    return (await ext.tabs.query({ active: true, currentWindow: true }))[0];
  }
  ...
  async function exportChat(openNew) {
    ...
    if (!id) {
      feedback.textContent = "Open a ChatGPT conversation first.";
      return;
    }
  ...
  toggle.addEventListener("change", async () => {
    ...
    if (toggle.checked) {
      const activeTab = await tab();
      await conversationId(activeTab, true);
    }
    await refresh();
  });
