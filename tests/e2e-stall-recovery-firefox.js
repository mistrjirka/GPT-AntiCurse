...
(async()=>{
  ...
  try {
    ...
    let s=await state(driver); assert.equal(s.sends,0); assert.equal(s.draft,""); let dbg=await driver.executeScript("return globalThis.CGAntiCurseStallRecovery?.debug?.()||null"); assert.equal(dbg?.recoveryPhase,"stopping"); assert.equal(dbg?.countdownRemainingMs,null); await waitFor(driver,"return window.__state.sends===1",5000); s=await state(driver); assert(s.sendAt-s.stopAt>=600);
